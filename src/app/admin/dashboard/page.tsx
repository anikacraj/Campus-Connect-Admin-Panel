...
const AdminDashboard = () => {
  ...
  const [universitiesRequest, setUniversitiesRequest] = useState<any[]>([]);
  const [filteredUniversitiesRequest, setFilteredUniversitiesRequest] = useState<any[]>([]);
  ...
  const menuItems = [
    { id: "allStudents", label: "Manage Students", icon: Users, color: "text-blue-600", hover: "hover:bg-blue-50" },
    { id: "allMods", label: "Manage Moderators", icon: Shield, color: "text-green-600", hover: "hover:bg-green-50" },
    { id: "allUniversity", label: "Manage Universities", icon: GraduationCap, color: "text-purple-600", hover: "hover:bg-purple-50" },
    { id: "allUniRequest", label: "University Requests", icon: Plus, color: "text-orange-600", hover: "hover:bg-orange-50" },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-6">
        {menuItems.map((item) => {
          ...
          return (
            <div
              key={item.id}
              onClick={() => router.push(`/admin/${item.id === "uniRequests" ? "allUniRequest" : `${item.id.replace(" ", "")}`}`)}
            ...
            >
              ...
              <p className="text-gray-500 text-sm">
                {item.id === "allStudents" && "Manage student accounts and activities"}
                {item.id === "allMods" && "Approve or remove moderator roles"}
                {item.id === "allUniversity" && "Monitor partnered universities"}
                {item.id === "allUniRequest" && "Review and approve new university requests"}
              </p>
            </div>
          );
        })}
